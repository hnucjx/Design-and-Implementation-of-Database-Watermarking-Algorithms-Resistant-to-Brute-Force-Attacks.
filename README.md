...
- 清晰度选择：默认 1080p；若视频不支持 1080p，会自动选用可用的最高分辨率。下载选项用一个“清晰度 / 格式”下拉同时支持分辨率策略和具体 `format_id`，具体格式会显示分辨率、fps、扩展名和文件大小，缺失时标注“大小未知”。明确选择 1440p、1080p、360p 等清晰度时，后端会要求实际下载高度匹配；无法匹配时任务失败，不再静默降级到低清。
