...
- 任务中心状态、进度、速度、实际分辨率、实际格式和显式删除入口展示。
- 任务中心播放已下载单视频和 playlist 子视频，打开视频所在文件夹和 playlist 文件夹，复制任务/子视频源链接，跳转 YouTube 页面，缺少输出文件时按钮禁用，本地文件操作失败时在对应任务行附近显示错误。
...
11. 下载完成后点击播放按钮，确认系统默认视频播放器打开对应文件；点击视频/合集文件夹按钮，确认文件管理器打开对应目录。Windows 下文件夹应新开 Explorer 窗口，而不是只在任务栏闪烁；若故意移动或删除文件，错误应显示在对应任务行或子视频行附近。
