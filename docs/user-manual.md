...
| 跳过已下载 | 默认开启，避免覆盖已有文件。 |
| 限速 KB/s（清空：不限速） | 默认空值，不限速；填写后会保存为全局运行时设置，并传给 yt-dlp 的 `ratelimit`。 |
| 重试次数 | 默认 10；修改后会保存为全局运行时设置，用于 yt-dlp 下载重试。 |

设置面板中的并发默认值为 5；如果网络连接不稳定，建议改为 1。并发、限速和重试次数都会在修改后立即应用，并显示“保存中...”“已保存”或“保存失败”：队列中和暂停中的任务会使用新值，正在下载的视频会取消当前 yt-dlp 实例并以断点续传方式重新入队，以便应用新的限速或重试次数。
