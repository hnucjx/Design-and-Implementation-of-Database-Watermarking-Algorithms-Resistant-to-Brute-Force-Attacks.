...
- 分辨率选择：支持 `best`、2160p、1440p、1080p、720p、480p，也支持分析后选择具体 `format_id`；具体格式会显示分辨率、fps、扩展名和文件大小，缺失时标注“大小未知”。
- 字幕下载：支持视频+字幕、仅视频、仅字幕；字幕语言为可搜索下拉多选；支持人工字幕、自动字幕或两者都要。
- Cookies 文件：支持手动上传/清除 `cookies.txt`，用于需要登录态或年龄验证的视频；不会在 UI 或日志中回显内容。
- 任务中心：支持单任务和批量暂停、重启、删除；实时显示百分比、已用时、预计剩余时间、下载速度、子项状态和失败原因。
...
3. 设置下载模式、分辨率、具体格式、字幕语言、字幕来源和字幕格式；具体格式下拉会展示对应分辨率和大小。
4. 按需开启 metadata、缩略图、跳过已下载、限速、重试和通知选项。
5. 点击“加入下载队列”，在任务中心查看实时进度。
6. 对单任务或多选任务执行暂停、重启、删除。
